...
node test/e2e/movefx.cjs       # 招式演出：62 招都放得完、大招周圍變暗、流星群 6 顆都落地、打中會頓一下震一下、減少震動的設定有效
...
桌面上的寶可夢維持原本的 **2 倍像素圖**，用程式讓它動起來（`src/renderer/gfx/rig.js`）：
自動找出每一隻的脖子（上半身最窄的那一列）和腳（最下面分成兩段的地方），切成頭、身體、左腳、右腳，每塊只移動整數個像素，
像原作早期「兩張圖交替」的動畫一樣，像素永遠是方的、不會糊掉，也不會出現原圖沒有的顏色。
待機時頭會慢慢上下（呼吸）；走路時左右腳輪流抬起往前跨、身體一上一下；會飄的整隻輕輕跳。每一隻的節奏不同步。
下載失敗時會先用依屬性顏色產生的替代圖。
...
心智不會自己選行為：選行為的仍然只有 `Pet.decide()`，心智只是把每個選項的權重乘上倍率（0.25–4）。
...
![最親近的那隻把樹果放在游標下面，坐在旁邊等](docs/screens/symbiosis-fruit.png)
...
- **休息的果實**：連續用電腦 90 分鐘（中間停不到 3 分鐘都算連續），最親近你的那隻把一顆樹果帶到游標正下方的螢幕下緣，坐在旁邊等。不催你，也不會變多。
  ...
  - 專注中不會放果實。
- **桌面的生氣**：今天做到幾件好事（有休息、完成一次專注、昨天準時睡），螢幕下緣的花草就長到幾級（0～3）。每天留下一朵小花排在下緣中間，最多 7 朵，像一週的足跡。
...
![在家的大家一起吃](docs/screens/symbiosis-eat.png)
![三件好事都做到：花草長滿下緣](docs/screens/symbiosis-bloom.png)
...
每一種寶可夢有 2～4 個招式（代表招式＋屬性招式，參考原作 X・Y），寫在 `src/renderer/scene/moves.js`，共 62 種演出：
...
3. 同一個東西錯開時間放好幾個：光束上一串光團各晚 0.03 秒冒出、脈衝環每 0.07 秒送出一個、流星每 0.13 秒落下一顆
4. 打到的瞬間爆開：光團放大 3 倍並淡出，加上打擊火花、衝擊波、往四周噴的亮線、各屬性的碎片，整個畫面頓一下再震一下

- 蓄力：光團從四周螺旋吸進嘴巴，中間越來越亮
- 光束：外層屬性色、中心白色，外面兩條旋轉的能量絲；電擊是一直變形的閃電
- 發射物：發光的核心＋不規則的外暈，一路留下漸淡的光團
- 衝撞：殘影和速度線；勇鳥猛攻全身包著火
- 擴散：一圈光團往外散、粗衝擊波、放射狀的光
- 保護：全身冒出往上升的光、光圈
